...
_prompt_command() {
  ...
  local machine="$(pcolor $user_color BOLD)\u$(pcolor)$(pcolor $HOST_COLOR BOLD)@\h$(pcolor)"
  local pwd="$(pcolor LBLUE BOLD)$(short_pwd)$(pcolor)"

  local env_info env_cmd cmd_result
  for env_cmd in "${ENV_INFO[@]}"
  do
    if [ $env_cmd == has_prodaccess_prompt ]; then
      continue
    fi
    cmd_result="$($env_cmd)"
  ...
  PS1="${last_command_info} ${shell_env}${prompt}"
